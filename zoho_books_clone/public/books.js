  ...
  <div class="zb-body">
    ...
    <div class="zb-detail-area" v-if="selectedName">
      ...
      <div class="zb-actionbar no-print" v-if="inv&&!detailLoading">
        ...
        <div style="display:flex;gap:6px">
          ...
          <template v-else>
            ...
            <button class="zb-ab-btn" @click="() => { const d = new frappe.ui.Dialog({ title: 'Send Invoice', fields: [{ fieldname: 'email', label: 'Email', fieldtype: 'Data', reqd: 1 }], primary_action_label: 'Send', primary_action(v) { d.hide(); toast('Invoice sent!','success'); } }); d.show(); }">
              <span v-html="icon('send',12)"></span> Send
            </button>
            <button class="zb-ab-btn" @click="() => {}">
              Share
            </button>
            <button class="zb-ab-btn" @click="printPdf">
              <span v-html="icon('print',12)"></span> PDF/Print
            </button>
            <button v-if="!isDraft" class="zb-ab-btn zb-ab-success">
              Record Payment
            </button>
            ...
            <button class="zb-ab-btn" @click="selectedName=null" title="Close">&#x2715;</button>
          </template>
        </div>
      </div>
      ...
      <div class="zb-banner no-print" v-if="inv&&!isDraft&&!editing">
        ...
        <span style="flex:1;font-size:12px"><b>WHAT'S NEXT?</b> Invoice has been sent. Record payment for it as soon as you receive payment.</span>
        <button class="zb-ab-btn zb-ab-success" style="font-size:11px;padding:4px 14px">Record Payment</button>
      </div>

      <!-- Loading -->
      <div v-if="detailLoading" style="flex:1;display:flex;align-items:center;justify-content:center">
        <div class="b-shimmer" style="width:200px;height:14px;border-radius:4px"></div>
      </div>
